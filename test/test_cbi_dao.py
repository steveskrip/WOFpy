...
class TestCbiDao(unittest.TestCase):
    ...
    def test_get_all_sites(self):
        ...
        for known_code in self.known_sites:
            self.assertTrue(known_code in resultSiteCodes, "allsites,known site<F3>Code not found:" + known_code)  # noqa

    def test_get_site_by_code(self):
        for known_code in self.known_sites:
            siteResult = self.dao.get_site_by_code(known_code)
            self.assertEqual(known_code, siteResult.SiteCode, "known siteCode not found:" + known_code)  # noqa

    def test_get_sites_by_codes(self):
        siteResultList = self.dao.get_sites_by_codes(self.known_sites)
        resultSiteCodes = [s.SiteCode for s in siteResultList]
        for known_code in self.known_sites:
            self.assertTrue(known_code in resultSiteCodes, ",known siteCode not found:" + known_code)  # noqa
    ...
    def test_get_all_variables(self):
        ...
        for known_code in self.known_var_codes:
            self.assertTrue(known_code in resultVarCodes, "known variable not found:" + known_code)  # noqa

    def test_get_var_by_code(self):
        for known_code in self.known_var_codes:
            varResult = self.dao.get_variable_by_code(known_code)
            self.assertEqual(known_code, varResult.VariableCode, "known variable not found:" + known_code)  # noqa
    ...
    def test_get_series_by_sitecode(self):
        for known_code in self.known_sites:
            ...
            self.assertNotEqual(len(seriesResultArr), 0, "no series found for:" + known_code)  # noqa
