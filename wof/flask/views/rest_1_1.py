...
try:
    ...
    rest_1_1 = Blueprint(__name__, __name__)
except ImportError:
    rest_1_1 = Module(__name__)
...
def index():
    ...
    return render_template('index_1_1.html',
                           ...
                           w=current_app.wof_inst.default_west,
                           so=current_app.wof_inst.default_south,
                           n=current_app.wof_inst.default_north,
                           e=current_app.wof_inst.default_east)


@rest_1_1.route('/GetSites', methods=['GET'])
def get_sites():
...
@rest_1_1.route('/GetSitesByBox', methods=['GET'])
def get_sites_by_box():

    west = request.args.get('west', None)
    south = request.args.get('south', None)
    north = request.args.get('north', None)
    east = request.args.get('east', None)
    IncludeSeries = request.args.get('IncludeSeries', False)

    siteResponse = current_app.wof_inst.create_get_site_box_response(west,south,north,east,IncludeSeries)

    if not siteResponse:
        return "Error: No sites found for box[(%s,%s),(%s,%s)]" % (west,east,south,north)
...
@rest_1_1.route('/GetSiteInfo', methods=['Get'])
def get_site_info():
    siteArg = request.args.get('site', None)
...
@rest_1_1.route('/GetSiteInfoMultiple', methods=['Get'])
def get_site_info_multiple():
    siteArg = request.args.get('site', None)

    siteInfoResponse = current_app.wof_inst.create_get_site_info_multiple_response(siteArg)

    if not siteInfoResponse:
        return "No site info found for site code [%s]" % (siteArg)
...
@rest_1_1.route('/GetVariableInfo', methods=['GET'])
...
@rest_1_1.route('/GetValues', methods=['GET'])
def get_values():
...
@rest_1_1.route('/GetValuesForASite', methods=['GET'])
def get_values_for_a_site():
    siteArg = request.args.get('site', None)
    startDateTime = request.args.get('startDate', None)
    endDateTime = request.args.get('endDate', None)

    if (siteArg == None):
        return "Must enter a site code (location)"

    timeSeriesResponse = current_app.wof_inst.create_get_values_site_response(
        siteArg, startDateTime, endDateTime)
...
@rest_1_1.route('/GetValuesWML2', methods=['GET'])
def get_values_wml2():
    """
